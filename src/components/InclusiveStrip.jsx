  ...
  return (
    <div className="px-[5%] py-16" style={{ background:'var(--navy)' }}>
      <div className="max-w-[1200px] mx-auto">
        <div className="f-mono text-[0.65rem] tracking-[0.2em] uppercase text-center mb-3" style={{ color:'var(--gold)' }}>{t.inclusive.label}</div>
        <h2 className="f-display font-black text-center text-white mb-12 tracking-[-0.01em]" style={{ fontSize:'clamp(1.8rem,3vw,2.8rem)' }}>{t.inclusive.h2}</h2>
        <div className="grid grid-cols-2 md:grid-cols-6 gap-[1px] rounded-lg overflow-hidden" style={{ background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.06)' }}>
          {inclItems.map(item => (
            <div key={item.name} className="text-center px-4 py-7 transition-colors hover:bg-white/[0.07]"
              ...
              <div className="f-display font-bold text-[0.85rem] tracking-[0.04em] uppercase mb-1" style={{ color: item.highlight ? 'var(--green-lt)' : 'white' }}>{item.name}</div>
