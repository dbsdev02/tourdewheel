  ...
  return (
    <section className="px-[5%] py-24" style={{ background:'white' }}>
      <div className="max-w-[1200px] mx-auto">
        <div className="f-mono text-[0.68rem] tracking-[0.2em] uppercase mb-3" style={{ color:'var(--blue)' }}>{te.label}</div>
        <h2 className="f-display font-black leading-none tracking-[-0.01em] mb-3" style={{ fontSize:'clamp(2.2rem,4vw,3.6rem)', color:'var(--navy)' }}>{te.h2}</h2>
        <p className="text-[1.05rem] font-light leading-[1.7] mb-10" style={{ color:'var(--mid)' }}>{te.sub}</p>
          ...
          {testis.map((t, i) => (
            <RevealItem key={i} className="testi-card">
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center f-display font-extrabold text-base text-white flex-shrink-0" style={{ background:'linear-gradient(135deg, var(--steel), var(--navy))' }}>{t.initial}</div>
                <div>
                  ...
                  <div className="text-[0.75rem] flex items-center gap-1.5" style={{ color:'var(--mid)' }}>
                    <span className="f-mono text-[0.6rem] tracking-[0.08em] uppercase px-1.5 py-[2px] rounded-sm" style={{ background:'rgba(27,79,114,0.08)', color:'var(--steel)' }}>{t.platform}</span>
                    · {t.city}
                  </div>
                </div>
              </div>
